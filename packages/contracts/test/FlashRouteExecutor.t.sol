...
abstract contract FlashRouteExecutorTest is Test {
    ...
    function _buildSingleHopV2_USDC_TO_WETH(uint256 amountIn, uint256)
        internal
        view
        returns (RouteParams memory params)
    {
...
contract MockBalancerVault is Test {
    ...
    function flashLoan(
        ...
        MockERC20(tokens[0]).transfer(recipient, repayAmount);
    }
}
...
contract MockAavePool is Test {
    ...
    function flashLoan(
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            MockERC20(assets[i]).mint(borrower, amounts[i]);
        }

        FlashRouteExecutor(payable(borrower)).executeOperation(
            assets, amounts, premiums, borrower, params
        );
    }
}
...
contract MockUniswapV2Router is Test {
    ...
    function swapExactTokensForTokens(
        ...
        outputToken.mint(to, swapOutputAmount);
...
contract MockUniswapV3Router is Test {
    uint256 public swapOutputAmount = 1e18;
    address public outputToken = address(0);
    ...
    function exactInput(IV3SwapRouter.ExactInputParams calldata p) external returns (uint256) {
        if (outputToken != address(0)) {
            MockERC20(outputToken).mint(p.recipient, swapOutputAmount);
        }
...
contract FlashRouteExecutor_ExecuteArbitrage is FlashRouteExecutorTest {
    function test_singleHopV2_profitableSwap() public {
        ...
        uint256 profit = 50e6;
        uint256 executorFunds = 2000e6;

        usdc.mint(address(mockBalancerVault), flashLoanAmount);
        usdc.mint(address(this), executorFunds);
        usdc.transfer(address(executor), executorFunds);
    ...
    function test_v3Hop_withSqrtPriceLimitX96() public {
        ...
        mockV3Router.setOutputToken(address(usdc));
    ...
    function test_revert_unprofitableRoute_insufficientProfit() public {
        ...
        params.minProfit = 1000e6;
        params.hops[0].tokenOut = address(usdc);

        vm.prank(operator);
        vm.expectRevert(abi.encodeWithSelector(Errors.InsufficientProfit.selector, 900000000, 1000000000));
        executor.executeArbitrage(params);
    }
    ...
    function test_profitSentToCorrectRecipient() public {
        ...
        RouteParams memory params = _buildSingleHopV2_USDC_TO_WETH(flashLoanAmount, flashLoanAmount + profit);
        params.minProfit = 0;
        params.hops[0].tokenOut = address(usdc);

        uint256 preProfitBal = usdc.balanceOf(profitRecipient);

        vm.prank(operator);
        executor.executeArbitrage(params);

        assertGt(usdc.balanceOf(profitRecipient), preProfitBal);
    }
